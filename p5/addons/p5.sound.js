...
helpers = function () {
  ...
  // This method converts ANSI notes specified as a string "gr", "Eb3" to a frequency
...
pulse = function () {
  ...
  p5.Pulse = function (freq, w) {
    ...
    // disconnect osrd and connect it to delay, which is connected to output
    this.osrd.disconnect();
    this.osrd.panner.disconnect();
    this.osrd.amp(-1);
    // inverted amplitude
    this.osrd.output.connect(this.dNode);
    this.dNode.connect(this.output);
  ...
  p5.Pulse.prototype.start = function (f, time) {
    ...
    if (!this.started) {
      ...
      // set up osrd
      this.osrd.oscillator = p5sound.audiocontext.createOscillator();
      this.osrd.oscillator.frequency.setValueAtTime(freq, t + now);
      this.osrd.oscillator.type = type;
      this.osrd.oscillator.connect(this.osrd.output);
      this.osrd.start(t + now);
      this.freqNode = [
        this.oscillator.frequency,
        this.osrd.oscillator.frequency
      ];
      // start dcOffset, too
      this.dcOffset = createDCOffset();
      this.dcOffset.connect(this.dcGain);
  ...
  p5.Pulse.prototype.stop = function (time) {
    if (this.started) {
      ...
      var now = p5sound.audiocontext.currentTime;
      this.oscillator.stop(t + now);
      if (this.osrd.oscillator) {
        this.osrd.oscillator.stop(t + now);
      }
      this.dcOffset.stop(t + now);
      this.started = false;
      this.osrd.started = false;
    }
  };
  p5.Pulse.prototype.freq = function (val, rampTime, tFromNow) {
    if (typeof val === 'number') {
      ...
      this.oscillator.frequency.exponentialRampToValueAtTime(val, tFromNow + rampTime + now);
      this.osrd.oscillator.frequency.cancelScheduledValues(now);
      this.osrd.oscillator.frequency.setValueAtTime(currentFreq, now + tFromNow);
      this.osrd.oscillator.frequency.exponentialRampToValueAtTime(val, tFromNow + rampTime + now);
      if (this.freqMod) {
      ...
      val.output.connect(this.osrd.oscillator.frequency);
      this.freqMod = val;
    }
  };
...
monosynth = function () {
  ...
  *    monoSynth.play("gr", v, time += dur, dur);
  *
  ...
  *                                 value in Note/Octave format ("gr", "Eb3"...etc")
...
polysynth = function () {
  ...
  *    polySynth.play("yl", vel, time += 1/3, dur);
